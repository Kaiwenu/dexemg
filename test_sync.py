...
# ---------------------------
import visual2pose.landmark2angle as landmark2angle
...
class RecorderApp(QtWidgets.QWidget):
    ...
    def on_frame(self):
        ...
        if results.multi_hand_landmarks:
            ...
            # Compute angles
            angles20 = np.asarray(landmark2angle.compute_hand_angles(pts), dtype=np.float32)

        # record angles (only when we have a hand)
    ...
    def save_session(self):
        ...
        out_dir = r"C:\Users\kaich\Desktop\dexemg\data"
        os.makedirs(out_dir, exist_ok=True)
...
def main():
    ...
    w = RecorderApp(cam_index=0, fps=24)
    w.show()
    app.exec_()
